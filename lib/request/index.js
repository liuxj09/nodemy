module.exports = function(config){
    ...
    let deviceAgent = ctx.request.header['user-agent'].toLowerCase();
		...
		ctx[questName] = (url = '', config = {}) => {
			...
			return new Promise((resolve, reject) => {
                ...
                if(data.code == 401){
                  let redirecturl = '';
                  if(isMobile){
                    // let redirect = `https://m.${ctx._host}/2018gystj/act`;
                    // redirect = encodeURIComponent(redirect);
                    redirecturl = `https://m.${ctx._host}/user/login`;
                  }else{
                    // let redirect = `https://act.${ctx._host}/2018gystj`;
                    redirecturl = `https://login.${ctx._host}`;
                  }
